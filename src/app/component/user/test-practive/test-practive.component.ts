...
import { Observable, Subscription, interval, iif, of } from 'rxjs';
...
export class TestPractiveComponent
  ...
  constructor(
    ...
    this.activatedRoute.params.forEach(urlParams => {
      ...
      this.examUsertID = urlParams['param2'];
  ...
  ngOnInit() {
    ...
    this.examService
      .getExamReSultByExamAndId(this.examUsertID, this.idExam)
      .pipe(tap(res => {
        if (res['completed'] === 2) {
          this.isOver = true;
          this.router.navigate([
            '/hometotal/testresult',
            { param1: this.idExam, param2: this.examUsertID }
          ]);
        }
        console.log(res);
        ...
        this.completedExam=res['completed'];
        if ( res['completed'] ===1) {
          this.tested = true;
       }
      }), concatMap( _ => {
        let action;
        if(this.completedExam!==2){
        if (this.tested === false ) {

       action = this.examService.getListQuestionExam(this.idExam);
         } else {

     action= this.examService.getListQuestionExamDetail(this.idExam.toString(), this.examUsertID.toString());
            }
          }
      return action;
      ...
      ).subscribe((res1: Object[]) => {

          this.listQuestion = res1;
          this.len = res1.length;
          //
          if (this.len > 0) {
            this.currentQuestion = res1[0];
            this.sortAnwserOption();
            this.listQuestion.forEach(element => {
              // tslint:disable-next-line:prefer-const
              let array: Object[] = [];
              this.mapAnswer.set(element['id'], array);
              // this.insertExamAnswer(element['id'], []);
            });
        }
        if (this.tested === false) {
          this.insertAllExamAnswerDefault(this.listQuestion).subscribe();
      } else {
        this.examService
        .getListQuestionResult(this.examUsertID)
        .subscribe(res2 => {
          if (res2.length > 0) {
            // danh sách câu trả lời của  các câu hỏi
            this.listAnswer = res2;
  ...
  clickScoreExam() {
    ...
    this.notinaiyo = 'Bạn đã làm ' + lenAnswered + '/' + this.len + ' câu hỏi! Bạn có muốn nộp bài không ?';

    } else {
      this.notinaiyo = 'Bạn có muốn nộp bài không ?';
    }
    this.notificationVisibilityWhenDelete = true;
    this.lenAnswered1 = lenAnswered;
  }
  oncg(event: boolean) {
    if (event) {
      this.isOver = true;
  ...
  crudAnswerExam(before: number[], now: number[]) {
    const data = {
      examUserId: this.examUsertID,
      complete: this.completedExam,
      question_id: 0,
      answer: [],
      time: this.displayTime,
      lenAnswered: this.lenAnswered1
    };
...
//  insertExamAnswer(id: number, array: number[]) {
//   const data = {
//     examUserId: this.examUsertID,
//     complete: 0,
//     question_id: id,
...
//   const formData = new FormData();
//   formData.append('data', JSON.stringify(data));
//   this.examService.insertExamAnswer(formData).subscribe();
//  }
 insertAllExamAnswerDefault(inputObject) {
  const data = {
    examUserId: this.examUsertID,
    complete: 0,
    question_id: 0,
  ...
  inputObject.forEach((object, key) => {
    const formData = new FormData();
    data.question_id = object['id'];
    formData.append('data', JSON.stringify(data));
    observableBatch.push(this.examService.insertExamAnswer(formData));
  });
