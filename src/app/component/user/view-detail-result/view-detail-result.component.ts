...
export class ViewDetailResultComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.examService.getExam(this.idExam).subscribe(res2 => {
      this.subjectName=res2['subject']['name'];
    });
    this.examService
    .getExamReSultByExamAndId(this.examUserID, this.idExam).pipe(tap(res=>{
     this.examCode= res['exam']['examMode'];
     this.typeExam= res['exam']['type'];
    }),concatMap( _ => {
        let action ;
        if(this.examCode===0 && this.typeExam===1){
        this.listQuestion =  JSON.parse(sessionStorage.getItem('listQuestion'));
        this.listAnswer =  JSON.parse(sessionStorage.getItem('listExamAnswer'));
        ...
        this.listAnswer.forEach(element => {
          this.mapAnswer.set(element['question_id'], element['choose_answer']);
          this.mapCheckQuestion.set(
            element['question_id'],
            this.checkCorrect(
              element['question_id'],
              this.mapQuesTion.get(element['question_id'])
            )
          );
        });
      }
        } else {
          action =this.examService.getListQuestionExamDetail(this.idExam.toString(),this.examUserID.toString());
        }
        return action;
    ...
    // this.examService.getListQuestionExamDetail(this.idExam.toString(),this.examUserID.toString()).subscribe(res => {
    //   this.listQuestion = res;
    //   this.len = this.listQuestion.length;
    //   this.listQuestion.forEach(element => {
    //     this.mapQuesTion.set(element['id'], element);
