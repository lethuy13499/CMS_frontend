...
export class ExamDetailUserComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.examService
      ...
      .pipe(
        tap(ress => {
          this.questionService.getQuestionOfExam(ress.id).subscribe(listQ => {
    ...
    this.examService.getChaptersNameByExamId(this.idExam).subscribe(res => this.chapters= res);
    this.examService.getDomainsNameByExamId(this.idExam).subscribe(res => this.domains= res);
  }
  onBack() {
    this.location.back();
  }
