...
export class ViewnewslistComponent implements OnInit {
  ...
  onViews(id) {
    ...
    var newWindow = window.open('/hometotal/pagenews/news/viewnews/' + id)
  }
  ...
  confirmDeleteClick() {
    if (this.news1.upStatus === 'reject') {
      this.news1.activeStatus = 'true';
      this.news1.pinned = 'true';
      this.news1.upStatus = 'approve';
    } else if (this.news1.upStatus === 'approve') {
      this.news1.upStatus = 'false';
      this.news1.pinned = 'false';
      this.news1.upStatus = 'reject';
    }
    this.viewlistnews.updateViewnNewsList(+this.news1.id, this.news1).subscribe();
