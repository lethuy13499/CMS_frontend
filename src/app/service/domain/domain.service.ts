...
export class DomainService {
  ...
  getDomainBySubjectId(id:number) {
    const tk = localStorage.getItem('access_token');
    let head = new HttpHeaders();
    head = head
      .set('TOKEN', 'Token' + tk);
    return this.http.get<domain[]>(Constant.API_GET_ALL_DOMAIN_BYSUBJECT_ID + '/' +id, {
      headers: head
    });
  }
  getAllListDomain(sort:Sort): Observable<Object>{
    const tk = localStorage.getItem('access_token');
    let head = new HttpHeaders();
    head = head
      .set('TOKEN', 'Token' + tk);
    return this.http.post(Constant.API_GET_LIST_DOMAIN,sort,{
      headers: head
    });
  }
