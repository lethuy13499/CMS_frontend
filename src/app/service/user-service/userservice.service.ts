...
export class UserserviceService {
  ...
  //Linh
  getListExamComplete(userid: number){
    const tk = localStorage.getItem('access_token');
    let head = new HttpHeaders();
    head = head.set('TOKEN', 'Token' + tk);
    return this.http.get<Object[]>(
      Constant.API_GET_PRODUCT_EXAM_COMPLETED + '/' + userid,
      {
        headers: head
      }
    );
 ...
 getListPracticeCompletedByUser(userId: number){
    const tk = localStorage.getItem('access_token');
    let head = new HttpHeaders();
    head = head.set('TOKEN', 'Token' + tk);
    return this.http.get<Object[]>(
      Constant.API_GET_PRACTICE_COMPLETED_BY_USER + '/' + userId,
      {
        headers: head
      }
    );
  ...
  getProductComingSoon(userId: number){
    const tk = localStorage.getItem('access_token');
    let head = new HttpHeaders();
    head = head.set('TOKEN', 'Token' + tk);
    return this.http.get<Object[]>(
      Constant.REST_API_GET_PRODUCT_COMING_SOON + '/' + userId,
      {
        headers: head
      }
    );
  ...
  getListExamCompletedByUser(userId: number){
    const tk = localStorage.getItem('access_token');
    let head = new HttpHeaders();
    head = head.set('TOKEN', 'Token' + tk);
    return this.http.get<Object[]>(
      Constant.REST_API_GET_EXAM_COMPLETED_BY_USER + userId ,
      {
        headers: head
      }
    );
  ...
  getListExamByUserAndSubject(userid: number, subjectId: number){
    const tk = localStorage.getItem('access_token');
    let head = new HttpHeaders();
    head = head.set('TOKEN', 'Token' + tk);
    return this.http.get<Object[]>(
      Constant.REST_API_GET_EXAM_COMPLETED_BY_USER_AND_SUBJECT + '?userid=' + userid + '&subjectId=' + subjectId ,
      {
        headers: head
      }
    );
  }
  getPracticeOfUser(userId: number) {
    const tk = localStorage.getItem('access_token');
    let head = new HttpHeaders();
    head = head.set('TOKEN', 'Token' + tk);
    return this.http.get<Object[]>(
      Constant.GET_PRACTICE_OF_USER + '?userId=' + userId,
      {
        headers: head
      }
    );
